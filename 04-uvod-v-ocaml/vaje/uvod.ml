...
let rec multiply sez = 
  match sez with
  | [] -> 1
  | prvi::ostalo -> prvi * multiply ostalo
...
(*domača naloga funkcija , ki naredi seznam ktih njev*)
...
let rec max_on_components l1 l2 = 
  ...
  | ([], _) -> []
  | (_, []) -> []
  | (x::xs, y::ys) -> (if x > y then x else y)::max_on_components l1 l2
