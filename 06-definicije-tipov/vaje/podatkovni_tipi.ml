...
let ratio = 0.84
let dollar_to_euro (Dollar a) = Euro (a *. ratio)
let euro_to_dollar (Euro a) = Dollar (a /. ratio)
...
let rec intbool_map f_int f_bool list = match list with
    | Empty -> Empty
...
let rec intbool_reverse list = 
    let rec aux l1 l2 = match l1 with
        | Empty -> l2
        | Int (x, xs) -> aux xs (Int (x, l2))
        | Bool (x, xs) -> aux xs (Bool (x, l2))
    in aux list Empty
...
let rec intbool_separate list = 
    let rec aux list ints bools = match list with
        | Empty -> (ints, bools)
        | Int(x, xs) -> aux xs (x::ints) bools 
        | Bool(x, xs) -> aux xs ints (x::bools) 
    in
...
let update ({arcane} as magic_counter) magic_type = match magic_type with
    | Fire -> { fire=magic_counter.fire + 1; 
                frost=magic_counter.frost + 1; 
                arcane=magic_counter.arcane + 1 }
    | Frost -> {magic_counter with frost=magic_counter.frost}
    | Arcane -> {magic_counter with arcane=arcane + 1}
...
let rec count_magic list = 
    let rec aux counter l = match l with
        | [] -> counter
        | x::xs -> 
            let counter' = match x.status with
                | Newbie -> counter
...
let zahtevana_leta spe = match spe with
    | Historian -> 3
    | Teacher -> 5
    | Researcher -> 4

let rec find_candidate mag spe list = match list with
    | [] -> None
    | wizard::rest -> (
        match wizard.status with
        | Student (stud_mag, years) when stud_mag=mag && 
            zahtevana_leta spe <= years -> Some wizard.name
        | _ -> find_candidate mag spe rest
    )
